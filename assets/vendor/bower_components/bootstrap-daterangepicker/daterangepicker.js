! function(t, e) {
   ...
   return a.prototype = {
      ...
      renderCalendar: function(a) {
         ...
         w += "<thead>", w += "<tr>", (this.showWeekNumbers || this.showISOWeekNumbers) && (w += "<th></th>"), w += C && !C.isBefore(i.firstDay) || this.linkedCalendars && "left" != a ? "<th></th>" : '<th class="prev available"><i class="fa fa-' + Y.left + " glyphicon glyphicon-" + Y.left + '"></i></th>';
         ...
         if (w += '<th colspan="5" class="month">' + P + "</th>", w += b && !b.isAfter(i.lastDay) || this.linkedCalendars && "right" != a && !this.singleDatePicker ? "<th></th>" : '<th class="next available"><i class="fa fa-' + Y.right + " glyphicon glyphicon-" + Y.right + '"></i></th>', w += "</tr>", w += "<tr>", (this.showWeekNumbers || this.showISOWeekNumbers) && (w += '<th class="week">' + this.locale.weekLabel + "</th>"), e.each(this.locale.daysOfWeek, function(t, e) {
               w += "<th>" + e + "</th>"
         ...
         for (var y = 0; y < 6; y++) {
            w += "<tr>", this.showWeekNumbers ? w += '<td class="week">' + i[y][0].week() + "</td>" : this.showISOWeekNumbers && (w += '<td class="week">' + i[y][0].isoWeek() + "</td>");
            for (var k = 0; k < 7; k++) {
               var N = [];
               ...
               H || (j += "available"), w += '<td class="' + j.replace(/^\s+|\s+$/g, "") + '" data-title="r' + y + "c" + k + '">' + i[y][k].date() + "</td>"
            }
            w += "</tr>"
         }
